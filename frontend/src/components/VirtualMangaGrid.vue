...
/**
 * 按行虚拟化的漫画网格，零依赖。
 * 只渲染可视区附近的行，漫画数量多时也能保持流畅；
 * 滚动节流 + 空闲时间预加载周边封面，避免滚动时集中请求图片。
 */
...
/** 可视区外、上下各 PREFETCH_ROWS 行的条目，空闲时预加载 */
const prefetchItems = computed(() => {
  ...
  const to =
    Math.min(rowCount.value, lastVisibleRow.value + PREFETCH_ROWS) * columns.value
...
<template>
  <div ref="viewport" class="virtual-grid" @scroll.passive="onScroll">
    <div class="virtual-grid-spacer" :style="{ height: `${totalHeight}px` }">
      ...
      >
        <MangaCard
          v-for="manga in visibleItems"
          :key="manga.id"
          :manga="manga"
          :cover-width="coverPixelWidth"
